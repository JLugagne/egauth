...
// Seal encrypts plaintext with a fresh random nonce, returning nonce||ciphertext||tag. The
// result is safe to store in the database.
...
// Open reverses Seal. It returns ErrCiphertextCorrupt if the blob is too short or the
// authentication tag does not verify (wrong KEK or tampering).
func (k *KEK) Open(sealed []byte) ([]byte, error) {
	ns := k.aead.NonceSize()
	if len(sealed) < ns {
