...
func TestService(t *testing.T) {
	...
	t.Run("Create and Validate", func(t *testing.T) {
		mockStore := &storetest.MockStore{
			CreateSessionFunc: func(ctx context.Context, tID string, sess *sessions.Session) error {
				return nil
			},
		...
		sess, token, err := svc.CreateSession(ctx, tenantID, userID, "UA", "127.0.0.1", time.Hour)
		require.NoError(t, err)
		assert.NotEmpty(t, token)
		assert.Equal(t, userID, sess.UserID)

		found, err := svc.ValidateSession(ctx, tenantID, token)
		require.NoError(t, err)
	...
	t.Run("Create Session Store Error", func(t *testing.T) {
		...
		svc := sessions.NewService(mockStore)
		_, _, err := svc.CreateSession(ctx, tenantID, userID, "UA", "127.0.0.1", time.Hour)
