...
// TestVerifyAccessTokenSingleTenantStillWorks confirms single-tenant verification keeps working:
// a verifier with no per-tenant configuration verifies a token through
// VerifyAccessTokenForTenant(ctx, "", token), so existing single-tenant callers are unaffected.
