...
## [ ] I5 — Authenticated change-email flow (verify-before-switch)
**Where:** `identity/service.go` + handlers. **Problem:** no ChangeEmail; only internal Store.UpdateUser. **Fix:** request → token to NEW address → confirm → atomic swap + reset EmailVerifiedAt. **Test:** swap only after new-address token confirmed.
